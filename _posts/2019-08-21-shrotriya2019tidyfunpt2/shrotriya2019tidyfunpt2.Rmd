...
In a custom In this case I needed to generate several $\LaTeX$ `newcommand`
entries of the form:
...
$\mathbf{A}$ i.e the uppercase equivalent.

Specifically I needed to construct 52 such combined sequential entries for both
lowercase/uppercase versions of these `newcommand` $\LaTeX$ macros. Rather than
do this manually, I realized that this would be fun scripting exercise with
using the `tidyverse` packages `glue`, `purrr`, and `stringr` similar to this
