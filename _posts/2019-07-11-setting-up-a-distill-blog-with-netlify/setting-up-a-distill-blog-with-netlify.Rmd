...
date: 07-11-2019
preview: images/08_06_netlify_adding_custom_domain.png
...
is based on the relatively new [distill](https://rstudio.github.io/distill/){target="_blank"}
package by the [RStudio](h//www.rstudio.com/){target="_blank"} team. The main tools used I used
...
- My macbook pro (2017 edition) and the [iterm2](https://www.iterm2.com/){target="_blank"} terminal
...
Importantly, I should note that the RStudio `distill` team has already
created an [excellent distill blog creation tutorial](https://rstudio.github.io/distill/blog.html){target="_blank"} which I thoroughly used
and highly recommend to new users. I wrote this meta blogpost in my own words
so that I can personally remember the details going forward. I also added more
details on deployment with Google Domains and Netlify that would hopefully be
useful to new `R` users waiting to deploy a similar `distill` blog.
...
<center>
```{r step-01-01-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
...
<center>
```{r step-01-02-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
...
Now I simply changed into the newly cloned blog directory using the following
command:

```{bash step-02-02, eval=FALSE, echo=TRUE, cache=TRUE}
...
```{r step-03-03, eval=FALSE, echo=TRUE, cache=TRUE}
...
[github pages](https://pages.github.com/){target="_blank"} down the line if needed.  You can
omit this if you don't want the option to have *github pages* as your host in
the future. I will be using `Netlify` to host my blog (see below), but it is
good to have an additional host option in the future.

My local `distill` blog directory now looked like this:
...
sort all future blogposts chronologically. Although I can could've changed this
manually. For simplicity I opted to delete the default welcome directory
altogether and recreated it with the date prefix as I prefer as detailed below.
...
```{r step-04-02, eval=FALSE, echo=TRUE, cache=TRUE}
...
<center>
```{r step-05-02-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
...
There are free alternatives e.g.  [Github Pages](https://pages.github.com/){target="_blank"},
but I wanted to have ownership on my page and found the annual fee to be
reasonable with Google Domains.
...
[netlify](https://www.netlify.com/){target="_blank"}! This is a free (and awesome) deployment
service. I created a personal account following the intructions on Netlify
website.
...
```{r step-08-01-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
...
<center>
```{r step-08-02-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
...
```{r step-08-03-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
                                     "08_03_netlify_deploy_master_branch_blog.png"))
```

<aside> This simply means that netlify will only build the site based on what
happens in my `master` branch in my [github
...
<center>
```{r step-08-04-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
...
<center>
```{r step-08-05-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
...
<center>
```{r step-08-06-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
...
<center>
```{r step-08-07-screenshot, fig.cap="Getting Started with Disqus", echo=FALSE, out.extra="class=external", out.width="150%", cache=TRUE}
knitr::include_graphics(path =
                            fs::path(IMAGES_PATH,
